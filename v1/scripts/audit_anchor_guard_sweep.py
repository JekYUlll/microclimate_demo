...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize fit-selected proxy anchor guards.")
...
def load_rows(path: Path) -> pd.DataFrame:
    ...
    frame["threshold"] = frame["controller_config"].map(
        lambda value: float(value["anchor_improvement_threshold"])
    )
    frame["base_controller"] = frame["controller_id"].str.split("_t", n=1).str[0]
    return frame
        ...
        rows.append(
            {
                ...
                "hard_constraint_violations": int(group["constraint_violation_count"].sum()),
...
def main() -> None:
    ...
    calibration = load_rows(root / "risk_calibration" / "window_risk_rows.jsonl")
    ...
    selected_fit = fit_summary[fit_summary["controller_id"] == selected_id].iloc[0]
    ...
    fit_summary.to_csv(root / "anchor_guard_fit_summary.csv", index=False)
    calibration_summary.to_csv(root / "anchor_guard_calibration_summary.csv", index=False)
    result = {
        ...
        "selection_rule": "fit max margin_q25, then mean, min, controller_id",
        "thresholds": sorted(float(x) for x in fit["threshold"].unique()),
        "base_controllers": sorted(str(x) for x in fit["base_controller"].unique()),
    }
    (root / "anchor_guard_gate.json").write_text(
